...
const baseSelect = "SELECT p.id,p.product_code AS productCode,p.category_id AS categoryId,p.product_name AS productName,p.brand,p.description,p.status,p.views,p.enquiries,p.created_at AS createdAt,p.updated_at AS updatedAt,c.name AS categoryName,(SELECT image_url FROM product_images WHERE product_id=p.id ORDER BY is_primary DESC,sort_order LIMIT 1) imageUrl FROM products p JOIN categories c ON c.id=p.category_id";
...
exports.create = async (req, res) => {
  ...
  const productName = productService.text(req.body.productName, "Product name", 2, 180), brand = productService.text(req.body.brand, "Brand", 2, 120), description = productService.text(req.body.description, "Description", 10, 5000), image = productImage(req.body.image, true);
  ...
  try {
    ...
    const [result] = await connection.execute("INSERT INTO products (product_code,seller_id,category_id,product_name,brand,description,status) VALUES (?,?,?,?,?,?,'active')", [temporaryCode, req.user.id, categoryId, productName, brand, description]);
    ...
    res.status(201).json({ message: "Product created successfully.", product: { id: result.insertId, productCode: code, categoryId, productName, brand, description, status: "active" } });
...
exports.update = async (req, res) => {
  const categoryId = positiveInt(req.body.categoryId, 0, Number.MAX_SAFE_INTEGER);
  if (!categoryId) throw new ApiError(400, "Category is required.", "CATEGORY_REQUIRED");
  const productName = productService.text(req.body.productName, "Product name", 2, 180), brand = productService.text(req.body.brand, "Brand", 2, 120), description = productService.text(req.body.description, "Description", 10, 5000), status = req.body.status || "active", image = productImage(req.body.image);
  ...
  try {
    ...
    if (duplicates[0]) throw new ApiError(409, "You already have a product with this name in this category.", "DUPLICATE_PRODUCT");
    await connection.execute("UPDATE products SET category_id=?,product_name=?,brand=?,description=?,status=? WHERE id=?", [categoryId, productName, brand, description, status, req.params.id]);
