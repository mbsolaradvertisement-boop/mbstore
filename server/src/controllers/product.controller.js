...
exports.create = async (req, res) => {
  ...
  const productName = productService.text(req.body.productName, "Product name", 2, 180), description = productService.text(req.body.description, "Description", 10, 5000), availability=availabilityValue(req.body.availability), image = productImage(req.body.image, true);
  ...
  try {
    ...
    const [result] = await connection.execute("INSERT INTO products (product_code,seller_id,category_id,company_id,product_name,brand,description,availability,status) VALUES (?,?,?,?,?,?,?,?,'active')", [temporaryCode, req.user.id, categoryId, company.id, productName, brand, description, availability]);
    ...
    res.status(201).json({ message: "Product created successfully.", product: { id: result.insertId, productCode: code, categoryId, companyId: company.id, productName, brand, description, availability, status: "active" } });
...
exports.update = async (req, res) => {
  const categoryId = positiveInt(req.body.categoryId, 0, Number.MAX_SAFE_INTEGER);
  if (!categoryId) throw new ApiError(400, "Category is required.", "CATEGORY_REQUIRED");
  const productName = productService.text(req.body.productName, "Product name", 2, 180), description = productService.text(req.body.description, "Description", 10, 5000), availability=availabilityValue(req.body.availability), status = req.body.status || "active", image = productImage(req.body.image);
  ...
  try {
    ...
    if (duplicates[0]) throw new ApiError(409, "You already have a product with this name in this category.", "DUPLICATE_PRODUCT");
    await connection.execute("UPDATE products SET category_id=?,company_id=?,product_name=?,brand=?,description=?,availability=?,status=? WHERE id=?", [categoryId, company.id, productName, brand, description, availability, status, req.params.id]);
