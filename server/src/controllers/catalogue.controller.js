...
exports.list = async (req, res) => {
  ...
  const [rows]=await pool.execute(`SELECT p.id,p.product_code AS productCode,p.product_name AS productName,p.brand,p.description,p.availability,c.id AS categoryId,c.name AS categoryName,COALESCE(sp.company_name,'Seller') AS sellerCompany,CASE WHEN co.id IS NULL THEN NULL ELSE CONCAT('/companies/',co.id,'/logo') END AS companyLogoUrl,(SELECT image_url FROM product_images WHERE product_id=p.id ORDER BY is_primary DESC,sort_order LIMIT 1) AS imageUrl ${from} WHERE ${clause} ORDER BY ${order} LIMIT ${limit} OFFSET ${offset}`,params);
