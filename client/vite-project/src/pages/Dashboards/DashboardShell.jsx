...
const adminNavigation = [
  ["Dashboard", FiGrid], ["Company Management", FiShoppingBag],
  ["Seller Management", FiUsers], ["Customer Management", FiUsers],
...
export default function DashboardShell({ title, eyebrow, children }) {
  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-slate-900">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-70 flex-col bg-[#15171b] text-white transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex h-20 items-center justify-between border-b border-white/8 px-6"><Link to="/" className="flex items-center gap-3 text-xl font-black"><span className="grid size-10 place-items-center rounded-xl bg-emerald-400 text-[#15171b]"><FiShoppingBag /></span>MB Store</Link><button className="text-xl text-slate-400 lg:hidden" onClick={() => setSidebarOpen(false)}><FiX /></button></div>
      <nav className="flex-1 overflow-y-auto px-4 py-6"><p className="px-3 text-[11px] font-bold uppercase tracking-[.16em] text-slate-500">Admin modules</p><div className="mt-3 space-y-1">{adminNavigation.map(([label, Icon], index) => <button key={label} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm transition ${index === 0 && location.pathname === "/admin/dashboard" ? "bg-white/10 font-bold text-white" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}><Icon className="text-lg" />{label}{label === "Notifications" && <span className="ml-auto rounded-md bg-emerald-400/15 px-2 py-0.5 text-[10px] font-bold text-emerald-400">3</span>}</button>)}</div></nav>
