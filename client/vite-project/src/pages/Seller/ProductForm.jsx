...
export default function ProductForm() {
  ...
  const [categories, setCategories] = useState([]), [fields, setFields] = useState([]), [form, setForm] = useState(empty);
  ...
  useEffect(() => {
    ...
    getSellerProduct(id).then(async ({ data }) => {
      ...
      setForm({ categoryId: String(product.categoryId), productName: product.productName, brand: product.brand, description: product.description, status: product.status, attributes: product.attributes || {} });
  ...
  const validate = () => {
    ...
    if (form.brand.trim().length < 2) next.brand = "Enter at least 2 characters.";
    ...
    return !next.categoryId && !next.productName && !next.brand && !next.description && !next.image && !Object.keys(attributes).length;
  };
  const submit = async (event) => {
    ...
    setSaving(true);
    try {
      const payload = { ...form, categoryId: Number(form.categoryId), productName: form.productName.trim(), brand: form.brand.trim(), description: form.description.trim(), ...(image ? { image } : {}) };
  ...
  return <form onSubmit={submit} className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="rounded-2xl border bg-white p-6 shadow-sm"><h2 className="text-lg font-black">Basic Information</h2><div className="mt-5 grid gap-5 sm:grid-cols-2">
      ...
      <Field label="Brand" error={errors.brand}><input value={form.brand} onChange={(event) => change("brand", event.target.value)} className="input" /></Field>
    ...
    <section className="grid gap-5 sm:grid-cols-2">
      <label className={`rounded-2xl border border-dashed bg-white p-6 text-center ${errors.image ? "border-red-400" : "border-slate-300"}`}><FiImage className="mx-auto text-3xl text-slate-400" /><b className="mt-2 block">Product Image <span className="text-red-600">*</span></b><p className="mt-1 text-xs text-slate-500">Required · PNG, JPEG or WebP · maximum 200 KB</p>{(image || existingImage) && <img src={image || existingImage} alt="Product preview" className="mx-auto mt-3 h-28 max-w-full rounded-xl object-contain" />}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => selectImage(event.target.files?.[0])} className="mt-3 text-xs" />{imageName && !errors.image && <p className="mt-2 text-xs text-teal-700">{imageName}</p>}{errors.image && <p className="mt-2 text-xs text-red-600">{errors.image}</p>}</label>
