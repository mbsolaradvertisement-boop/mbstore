...
function ProductCard({ product, onDetails }) {
  return <article className="group flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-3 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-sky-200 hover:shadow-xl">
    <div className="placeholder-pattern relative aspect-[4/3] overflow-hidden rounded-xl border border-slate-200 text-slate-400">
      ...
      <span className="absolute left-3 top-3 z-10 flex max-w-[calc(100%-1.5rem)] items-center gap-1.5 rounded-full bg-white px-2.5 py-1 text-[10px] font-black text-sky-700 shadow-sm">
    ...
    <div className="flex flex-1 flex-col p-2 pt-4">
      ...
      <div className="mt-auto grid gap-2 pt-5 sm:grid-cols-2"><button type="button" onClick={()=>onDetails?.(product)} className="inline-flex items-center justify-center gap-1 rounded-lg border border-slate-200 px-2 py-2.5 text-xs font-extrabold text-slate-700 transition hover:border-sky-300 hover:text-sky-700">Details<FiArrowRight/></button><button className="inline-flex items-center justify-center gap-1 rounded-lg bg-red-600 px-2 py-2.5 text-xs font-extrabold text-white transition hover:bg-red-700"><FiMessageSquare/>Quote</button></div>
    </div>
  </article>;
}
